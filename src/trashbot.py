...
def upcomming(update, context):
    keyboard = [[InlineKeyboardButton("\U0001F4F0", callback_data='paper'),
        ...
        [InlineKeyboardButton("Cargo \U0001F68B", callback_data='cargotram'),
            InlineKeyboardButton("E - \U0001F68B", callback_data='etram')]]

    reply_markup = InlineKeyboardMarkup(keyboard)

    update.message.reply_text('What do you want to throw away?:', reply_markup=reply_markup)
...
def button(update, context):
    ...
    openerz = "http://openerz.metaodi.ch/api/calendar/%s.json?zip=%s&start=%s&offset=0&limit=1" % (query.data, "8006", datetime.today().strftime('%Y-%m-%d'))
    with urllib.request.urlopen(openerz) as url:
        data = json.loads(url.read().decode())
        next_date = date.fromisoformat(data['result'][0]['date']).strftime('%a %x')
        print(data)
    # date = "23. Oktober"; 

    query.edit_message_text(text="Next %s in your area: %s" % (name[query.data], next_date))
